...
# The full URL is pinned to an exact allowlist (any userinfo, port, query, or fragment
# fails the match). A pull request must not be able to point the harness's git clone/fetch
# at an arbitrary host (SSRF) or a non-upstream repo (validation-data substitution) —
# GHSA-8cfp-3c4q-xr6x.
