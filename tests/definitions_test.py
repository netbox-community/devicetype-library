...
def test_definitions(file_path, schema, change_type):
    ...
    if (definition.get('front_image') or definition.get('rear_image')):
        ...
        if(definition.get('front_image')):
            ...
            if not front_image:
                pytest.fail(f'{file_path} has front_image set to True but no matching front image found for device (looking for \'elevation-images{os.path.sep}{file_path.split(os.path.sep)[1]}{os.path.sep}{this_device.get_slug()}.front.ext\' but found {manufacturer_images})', False)
        ...
        if(definition.get('rear_image')):
            ...
            if not rear_image:
                pytest.fail(f'{file_path} has resr_image set to True but no matching rear image found for device (looking for \'elevation-images{os.path.sep}{file_path.split(os.path.sep)[1]}{os.path.sep}{this_device.get_slug()}.rear.ext\' but found {manufacturer_images})', False)
    iterdict(definition)
