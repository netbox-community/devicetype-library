...
class DeviceType:
    ...
    def verify_slug(self, KNOWN_SLUGS):
        ...
        known_slug_list_intersect = [
            (slug, file_path) for slug, file_path in KNOWN_SLUGS if slug == self.slug
        ]
        ...
        if len(known_slug_list_intersect) == 0:
            ...
            if self.file_path not in known_slug_list_intersect[0][1]:
                if "R" not in self.change_type:
                    self.failureMessage = (
                        f'{self.file_path} has a duplicate slug: "{self.slug}"'
                    )
                    return False
            return True
        else:
        ...
        if not (
            self.slug.endswith(self._slug_model)
            or self.slug.endswith(self._slug_part_number)
        ):
...
def validate_component_names(component_names: set or None):
    if len(component_names) > 1:
        ...
        for index, name in enumerate(component_names):
            ...
            intersection = sorted(
                set(verify_name) & set(list(name)), key=verify_name.index
            )
            ...
            subset_match = sorted(
                set(verify_subset) & set(name_subset), key=name_subset.index
            )
...
def verify_filename(device: DeviceType or ModuleType, KNOWN_MODULES: set or None):
    ...
    if not (
        filename == device._slug_model
        or filename == device._slug_part_number
        or filename == device.part_number.casefold()
    ):
    ...
    if not device.isDevice:
        matches = [
            file_name
            for file_name, file_path in KNOWN_MODULES
            if file_name.casefold() == filename.casefold()
        ]
        if len(matches) > 1:
        ...
        for name, position in known_components:
            if position is not None:
                ...
                if len(position_set) > 0:
                    match = [
                        key for key, val in position_set.items() if key == position
                    ]
                if len(match) == 0:
                    if len(position_set) == 0:
